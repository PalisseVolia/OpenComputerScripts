...
Matrix = component.induction_matrix
...
local function updateReactorPanel()
    ...
    EnergybarColor = 0xFFFFFF

    while running do
        -- Create the main window
        ReactorPanel = Window:new(15, 3, 50, 21)
        ...
        if EnergyRatio < 20 then
            EnergybarColor = 0xFF0000
        end
        if EnergyRatio > 20 then
            EnergybarColor = 0xFF8000
        end
        if EnergyRatio > 50 then
            EnergybarColor = 0xFFE600
        end
        if EnergyRatio > 70 then
            EnergybarColor = 0x00FF00
        end
        if EnergyRatio > 90 then
            EnergybarColor = 0x00FFFF
        end
        local EnergyText = TextLine:new(5, X+4, "Energy :")
        ReactorPanel:addChild(EnergyText)
        local energyProgressBar = ProgressBar:new(5, X+5, EnergyRatio, EnergybarColor)
        ...
        CurrentEnergy = math.floor(Matrix.getEnergy())
        ...
        if not manualMode then
            ...
            if HeatRatio < 50 and EnergyRatio > 99 then
                Reactor.deactivate()
            end
        end
